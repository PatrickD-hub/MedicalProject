...
from util import (compute_view_params, get_rotation_matrix_from_two_points,
                  meters2xyz, pr2t, r2w, rpy2r, trim_scale, r2quat, r2rpy)
...
class MuJoCoParserClass():
    """
        MuJoCo Parser class
    """
    def __init__(self,name='Robot',rel_xml_path=None,USE_MUJOCO_VIEWER=True,VERBOSE=True, tool_transform = np.eye(4)):
        """
        ...
        if np.array_equal(self.tool_transform, np.eye(4)):
            self.use_endeffector = False
        else:
            self.use_endeffector = True
    ...
    def _parse_xml(self):
        ...
        self.scene            = self.full_xml_path
        self.model            = mujoco.MjModel.from_xml_path(self.scene)
        self.data             = mujoco.MjData(self.model)
        self.data_lock        = self._get_data_lock()
        ...
        self.ctrl_qvel_idxs = []
        self.ctrl_qacc_idxs = [] # accelartion  index
        for ctrl_idx in range(self.n_ctrl):
            transmission_idx = self.model.actuator(self.ctrl_names[ctrl_idx]).trnid # transmission index
            ...
            self.ctrl_qacc_idxs.append(joint_idx) # accelartion
    ...
    def print_info(self):
        ...
        print ("ctrl_qacc_idxs:%s"%(self.ctrl_qacc_idxs))
        print ("ctrl_ranges:\n%s"%(self.ctrl_ranges))
    ...
    def set_initial_qpos(self, pos):
        """
        Set the initial qpos of the Mujoco model.

        Args:
            pos (list): List containing the desired joint positions.
                        Example: [-1.06811, -1.18511, 1.51268, -1.88439, -1.63357, -1.23787e-09]
        """
        # Ensure the length of the provided position matches the number of joints
        if len(pos) != len(self.data.qpos):
            raise ValueError("Length of 'pos' must match the number of joints in the model.")

        # Update the qpos
        self.data.qpos[:] = pos
        mujoco.mj_forward(self.model,self.data)

    def print_transform(self,body_name):
    ...
    def get_x(self):
        """
            Get body position and rotation in Euler XYZ
        """
        xpos, R = self.get_pR_body(self.body_names[len(self.joint_names)+1+self.use_endeffector])

        # xrot = r2rpy(R)
        xrot = r2w(R)

        return np.hstack((xpos, xrot))
    ...
    def get_dx(self):
        ...
        dq = self.get_dq()
        J = self.get_J()
        vel = np.linalg.pinv(J) @ np.transpose(dq)

        return vel.copy()
    
    def get_xF(self, KF, fd):
        """
            Returns a velocity reference
        """
        return KF * (fd - self.get_fe())
    ...
    def get_n(self):
        """
            Get Coriolis, centrifugal and gravitational forces
        """
        return np.asarray(self.data.qfrc_bias)
    ...
    def get_he(self):
        """
        Returns the external torque
        """
        he = self.get_J().T @ np.reshape(self.data.qfrc_applied,(6,))

        ee_R_t = self.tool_transform[:3,:3]
        ee_t_t = self.tool_transform[:3, 3]
        S_mat = np.array([[0, -ee_t_t[2], ee_t_t[1]],
                    [ee_t_t[2], 0, -ee_t_t[0]],
                    [-ee_t_t[1], ee_t_t[0], 0]])
    ...
    def get_dq(self,joint_idxs=None):
        """
            Get joint velocity in (radian/s)
        """
        if joint_idxs is None:
            dq = self.data.qvel
        else:
            dq = self.data.qvel[joint_idxs]

        return dq.copy()
    
    def get_J(self):
        """
            Get Jocobian matrix of end effector
        """
        J_p = np.zeros((3,self.model.nv)) # nv: nDoF
        J_R = np.zeros((3,self.model.nv))
        mujoco.mj_jacBody(self.model,self.data,J_p,J_R,self.data.body(self.body_names[len(self.joint_names)+1+self.use_endeffector]).id)
        J_full = np.array(np.vstack([J_p,J_R]))
        return J_full
    
    def get_T(self):
        """
            Get transform of the endeffector
        """
        p, R = self.get_pR_body(self.body_names[len(self.joint_names)+1+self.use_endeffector])
        T = np.eye(4)
        T[0:3,0:3] = R
        T[0:3, 3] = p
        return T
    ...
    def get_qacc_joints(self,joint_names):
        """
            Get multiple joint accelerations from 'joint_names'
        """
        return np.array([self.get_qacc_joint(joint_name) for joint_name in joint_names]).squeeze()
        ...
        with mujoco.viewer.launch_passive(self.model, self.data) as viewer:
            ...
            while viewer.is_running():
                with self.data_lock:
                    ...
                    self.update_viewer()
                    mujoco.mj_forward(m=self.model, d=self.data)
                    mujoco.mj_step(m=self.model, d=self.data)
